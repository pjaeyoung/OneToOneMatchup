    ...
    void Update ()
    {
        ...
        else if (scene.name == "GameScene")
        {
            if (interactive == true)
            {
                ...
                interactive = false;
                itemBtnCanvas.gameObject.transform.Find("btn_GetArmor").gameObject.SetActive(false);
                itemBtnCanvas.gameObject.transform.Find("btn_GetWeapon").gameObject.SetActive(false);
                for (int i = 0; i < 3; i++)
                    itemBtn[i].enabled = false;
                itemBtnCanvas.transform.Find("btn_GetItem").GetComponent<RectTransform>().anchoredPosition = new Vector2(-12, 342);
            }
        }
	}
